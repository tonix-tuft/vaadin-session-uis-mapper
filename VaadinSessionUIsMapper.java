 ...
 * to its attached UIs. UIs which want to be mappable must extend the VaadinAbstractMappableUI.
 * 
...
public class VaadinSessionUIsMapper {
	 ...
	 * @throws IllegalArgumentException If the given ui doesn't extend the VaadinAbstractMappableUI 
	 * 									or if super is not called during UI's construction
	 * @throws ClassCastException If the given ui parameter is not an AbstractMappableUI Vaadin UI
	 */
	public synchronized void putUI(String sessId, VaadinAbstractMappableUI ui) {
		if (!(ui instanceof VaadinAbstractMappableUI)) {
			throw new IllegalArgumentException(String.format("The given UI instance of class %s is not "
		...
		if (mappableUI.isOverriddenBad()) {
			throw new IllegalArgumentException(String.format("The given UI instance of class %s has not called super "
					+ " in its constructor overridings", 
					ui.getClass().getName()));
		}
		...
		if (cuis.contains(ui)) {
			for (UI cui : cuis) {				
				if (cui.getClass().getName().equals(ui.getClass().getName())) {
					System.out.println("Removing UI with class: "+cui.getClass().getName()+" for session ID = "+sessId);
					cuis.remove(cui);
					cuis.add((UI)ui);
					System.out.println("Adding UI to Session with ID: "+sessId+", UI class = "+ui.getClass().getName());
				}
			}
		}
		else {
			System.out.println("New UI added to Session with ID: "+sessId+", UI class = "+ui.getClass().getName());
			cuis.add((UI)ui);
		}
	...
	/**
	 * Associates the given Vaadin session (the session id) to the given UI collection. If the mapping contained a previous
